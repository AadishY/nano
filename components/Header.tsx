...
const Header: React.FC<HeaderProps> = ({ onLogoClick }) => {
  return (
    <header className="w-full py-4 px-8 border-b border-gray-700 bg-gray-800/30 backdrop-blur-sm sticky top-0 z-50">
      <button onClick={onLogoClick} className="flex items-center justify-center gap-3 mx-auto group" aria-label="Go to home screen">
          <SparkleIcon className="w-6 h-6 text-blue-400 transition-transform duration-300 group-hover:rotate-12" />
          <h1 className="text-xl font-bold tracking-tight text-gray-100 transition-colors group-hover:text-white">
            AadishPixShop
          </h1>
      </button>
    </header>
